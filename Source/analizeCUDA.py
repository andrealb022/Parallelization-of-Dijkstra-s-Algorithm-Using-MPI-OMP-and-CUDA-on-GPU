...
def analizeFile(sourceFile,resultFile):#analize the source file and save the information in the result file
    ...
    for line in lines:#for each line get the informations
        type,omp,mpi,totalTime,creationTime,allocationTimeGPU,executionTime=line.strip()[:-1].split(";")
        if type not in data:
            data[type]={}
            num[type]={}
        ...
        if mpi not in data[type][omp]:
            data[type][omp][mpi]=[float(totalTime),float(creationTime),float(allocationTimeGPU),float(executionTime)]
            num[type][omp][mpi]=1
        else:
            data[type][omp][mpi][0]+=float(totalTime)
            data[type][omp][mpi][1]+=float(creationTime)
            data[type][omp][mpi][2]+=float(allocationTimeGPU)
            data[type][omp][mpi][3]+=float(executionTime)
            num[type][omp][mpi]+=1
    f.close()#close the source file and open the result file
    f=open(resultFile,"w")
    f.write("Modality;OMP;CUDA;total time;graph creation time;GPU allocation time;Dijkstra execution time;Speedup;Efficiency \n")
        ...
        for ompKey in ompList:
            ...
            for mpiKey in mpiList:
                ...
                if typeKey =="Sequential":
                    ...
                    #efficiency
                    data[typeKey][ompKey][mpiKey].extend([par_time,par_time / int(ompKey)])
...
if os.path.exists("ResultsOMP+CUDA/FinalAnalysisDijkstra"):#if Result directory exist, delete it and all its elements
    shutil.rmtree('ResultsOMP+CUDA/FinalAnalysisDijkstra')

os.mkdir("ResultsOMP+CUDA/FinalAnalysisDijkstra")#create the result directory

for path, currentDirectory, files in os.walk("ResultsOMP+CUDA/InfoTimeDijkstra"):#for each file in the directory Informations, analize them and save the results in the relative path in Results directory 
    for file in files:
        resultPath=path.replace("ResultsOMP+CUDA/InfoTimeDijkstra","ResultsOMP+CUDA/FinalAnalysisDijkstra")
